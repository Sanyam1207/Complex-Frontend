...
export default function Page() {
    ...
    useEffect(() => {
        const fetchListingData = async () => {
            ...
            try {
                console.log(true);
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/rentals/${listingId}`);
                if (!response.ok) {
                ...
                setIntersection(data.nearestIntersection || '');
                setBedrooms(data.numberOfBedrooms?.toString() || '');
                setBathrooms(data.numberOfBathrooms?.toString() || '');
                setIsCoupleFriendly(!!data.coupleFriendly);
                setSelectedAmenities(data.amenities || []);
                setDescriptionPoints(data.description || ['']);
                setWalkingDistancePoints(data.walkingDistanceTo || ['']);
